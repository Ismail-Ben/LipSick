...
def main():
    ...
    video_frame_dir, video_size = extract_video_frames(opt)
    ...
    if len(video_frame_path_list_cycle) >= res_frame_length:
        ...
        divisor = res_frame_length // len(video_frame_path_list_cycle)
        remainder = res_frame_length % len(video_frame_path_list_cycle)
    ...
    if opt.activate_custom_frames:
        ref_index_list = parse_reference_indices(opt.custom_reference_frames) if opt.custom_reference_frames else random.sample(range(5, len(res_video_frame_path_list_pad) - 2), 5)
    else:
    ...
    for ref_index in ref_index_list:
        crop_flag, crop_radius = (True, opt.custom_crop_radius) if opt.custom_crop_radius and opt.custom_crop_radius > 0 else compute_crop_radius(video_size, res_video_landmark_data_pad[ref_index - 5:ref_index, :, :])
    ...
    res_video_path = get_versioned_filename(os.path.join(opt.res_video_dir, res_video_name))
    videowriter = cv2.VideoWriter(res_video_path, cv2.VideoWriter_fourcc(*'mp4v'), 25, video_size)

    if opt.auto_mask:
        samelength_video_path = get_versioned_filename(os.path.join(opt.res_video_dir, 'samelength.mp4'))
        videowriter_samelength = cv2.VideoWriter(samelength_video_path, cv2.VideoWriter_fourcc(*'mp4v'), 25, video_size)

    res_face_path = get_versioned_filename(os.path.join(opt.res_video_dir, os.path.basename(opt.source_video_path)[:-4] + '_facial_dubbing_face.mp4'))
    ...
    for clip_end_index in range(5, pad_length, 1):
        ...
        if opt.auto_mask:
            videowriter_samelength.write(frame_data[:, :, ::-1])
        ...
        crop_frame_h, crop_frame_w = crop_frame_data.shape[:2]
        ...
        with torch.no_grad():
            pre_frame = model(crop_frame_tensor, ref_img_tensor, deepspeech_tensor).squeeze(0).permute(1, 2, 0).detach().cpu().numpy() * 255
    ...
    video_add_audio_path = get_versioned_filename(os.path.join(opt.res_video_dir, 'pre_blend.mp4' if opt.auto_mask else os.path.basename(opt.source_video_path)[:-4] + '_LIPSICK.mp4'))
